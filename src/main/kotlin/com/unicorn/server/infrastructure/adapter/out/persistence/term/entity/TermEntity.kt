...
class TermEntity protected constructor() {
	...
	fun toDomain(): Term {
		val persistedId = requireNotNull(id) { "term_id가 없는(영속화 전) 엔티티입니다" }
		val persistedEffectiveAt = requireNotNull(effectiveAt) { "effective_at이 없습니다" }
		val persistedCreatedAt = requireNotNull(createdAt) { "created_at이 없습니다" }
		val persistedUpdatedAt = requireNotNull(updatedAt) { "updated_at이 없습니다" }
