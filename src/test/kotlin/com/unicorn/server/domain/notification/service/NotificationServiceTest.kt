...
class NotificationServiceTest {
    ...
    private val userNotificationService = NotificationService(notificationInboxOutPort)

    @Test
    @DisplayName("알림 보관함 조회 시 현재 멤버의 알림만 최신순으로 반환한다")
    fun getNotifications_onlyCurrentMemberNotifications_returnsLatestFirst() {
        ...
        val page = userNotificationService.getNotifications("member-1", cursor = null, size = 20)
    ...
    fun getNotifications_withMoreItems_returnsNextCursor() {
        ...
        val page = userNotificationService.getNotifications("member-1", cursor = null, size = 2)
    ...
    fun markNotificationAsRead_existingNotification_setsReadAt() {
        val item = notificationInboxOutPort.save(createItem(receiverMemberId = "member-1", dedupKey = "dedup-1"))

        val result = userNotificationService.markNotificationAsRead("member-1", requireNotNull(item.id).value)
    ...
    fun markNotificationAsRead_otherMemberNotification_throwsException() {
        val item = notificationInboxOutPort.save(createItem(receiverMemberId = "member-1", dedupKey = "dedup-1"))

        assertThatThrownBy {
            userNotificationService.markNotificationAsRead("member-2", requireNotNull(item.id).value)
        }
